...
class MainWindow(QMainWindow):
    ...
    def setup_core_components(self) -> None:
        ...
        # Create cognitive load calculator
        self.cognitive_load_calculator = CognitiveLoadCalculator(self.config)
    ...
    def update_ui(self) -> None:
        ...
        # Update camera widget if visible
    ...
    def on_task_completed(self, task_data: Dict[str, Any]) -> None:
        ...
        self.cognitive_load_calculator.update_performance_metrics(performance_metrics)
    ...
    def on_settings_changed(self, settings: Dict[str, Any]) -> None:
        ...
        if settings.get('facial_analysis', {}).get('enabled') != \
           self.config.get('facial_analysis', {}).get('enabled'):
            if settings.get('facial_analysis', {}).get('enabled'):
                self.facial_analyzer.start_analysis()
            else:
                self.facial_analyzer.stop_analysis()
        
        if settings.get('advanced_features', {}).get('eeg', {}).get('enabled') != \
           self.config.get('advanced_features', {}).get('eeg', {}).get('enabled'):
            if settings.get('advanced_features', {}).get('eeg', {}).get('enabled'):
                self.eeg_integration.connect()
            else:
    ...
    def closeEvent(self, event) -> None:
        ...
        # Stop components
        if self.facial_analyzer.is_analyzing:
            self.facial_analyzer.stop_analysis()
        
        if self.eeg_integration.is_connected:
            self.eeg_integration.disconnect()
